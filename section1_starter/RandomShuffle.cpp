...
string randomShuffle(string input) {
    if (input.length() == 1) {
        ...
        char randomChar = input[randomIndex];
        string restInput = input.substr(0, randomIndex) + input.substr(randomIndex + 1);
        return charToString(randomChar) + randomShuffle(restInput);
    }
}
